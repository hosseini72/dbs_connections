...
class MongoPoolConfig(BasePoolConfig):
    ...
    def __post_init__(self):
        ...
        if self.auth_mechanism:
            valid_auth_mechanisms = {
                "SCRAM-SHA-1", "SCRAM-SHA-256", "MONGODB-CR",
                "MONGODB-X509", "PLAIN", "GSSAPI"
            }
    ...
    def get_connection_params(self) -> Dict[str, Any]:
        ...
        elif self.w is not None or self.wtimeout is not None or self.journal is not None:
            if self.w is not None:
                params["w"] = self.w
            if self.wtimeout is not None:
        ...
        if self.tls or (self.ssl is not None and self.ssl):
            ...
            if self.tls_certificate_key_file or self.ssl_certfile:
                params["ssl_certfile"] = self.ssl_certfile or self.tls_certificate_key_file
                params["tlsCertificateKeyFile"] = self.ssl_certfile or self.tls_certificate_key_file
            if self.ssl_keyfile:
                params["ssl_keyfile"] = self.ssl_keyfile
            if self.tls_certificate_key_file_password:
                params["tlsCertificateKeyFilePassword"] = self.tls_certificate_key_file_password
