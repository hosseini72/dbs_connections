...
class MongoHealthChecker:
    ...
    def __init__(self, pool_or_config: Union["MongoSyncConnectionPool", "MongoAsyncConnectionPool", "MongoPoolConfig"]):
            ...
            try:
                ...
                return HealthStatus(
                    ...
                    details={
                        ...
                        "utilization_percent": round((current_conns / total_conns * 100) if total_conns > 0 else 0, 2),
                    },
                )
            except Exception:
    ...
    async def async_check_connection(self, connection: AsyncIOMotorClient) -> HealthStatus:
        """Async version: Check health of a single connection.
    ...
    async def async_check_pool(self) -> HealthStatus:
        ...
        try:
            if hasattr(self.pool, 'pool_status') and not asyncio.iscoroutinefunction(self.pool.pool_status):
                # Sync pool
                status = self.pool.pool_status()
            else:
                # Async pool
        ...
        try:
            with self.pool.get_connection() as client:
                ...
                try:
                    ...
                    details["replica_set_members"] = len(repl_set_status.get("members", []))
                except Exception:
                    # Not a replica set
                    pass
                ...
                try:
                    ...
                    details["database_size_mb"] = round(db_stats.get("dataSize", 0) / (1024 * 1024), 2)
        ...
        if connection:
            try:
                ...
                try:
                    ...
                    result["server"] = server_status if server_status is not None else {}
                except Exception:
                    result["server"] = {}
                try:
    ...
    async def async_comprehensive_check(self) -> dict:
        ...
        pool_status, db_status = await asyncio.gather(
            self.async_check_pool(),
            self.async_check_database(),
    ...
    async def async_check_server_status(self, connection: AsyncIOMotorClient) -> Optional[Dict[str, Any]]:
    ...
    def check_replication_status(self, connection: MongoClient) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            try:
                ...
                if repl.get("setName"):
                    return {
                        ...
                        "is_primary": repl.get("ismaster", False) or repl.get("isWritablePrimary", False),
            ...
            return {
                ...
                "is_primary": ismaster.get("ismaster", False) or ismaster.get("isWritablePrimary", False),
    ...
    async def async_check_replication_status(self, connection: AsyncIOMotorClient) -> Optional[Dict[str, Any]]:
